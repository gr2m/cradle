...
Database.prototype._save = function (id, rev, doc, callback) {
    ...
    if (Array.isArray(doc)) {
        ...
        // if _revisions is set on docs, we set new_edits to false
        // see http://wiki.apache.org/couchdb/HTTP_Bulk_Document_API#Posting_Existing_Revisions
        if (document.docs.length && document.docs[0]._revisions) {
          document.new_edits = false;
        }
        this.query({
            method: 'POST', 
            path: '/_bulk_docs', 
